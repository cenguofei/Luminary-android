 ...
 * ```
 */
class LunimaryHttpRequestRetry internal constructor(configuration: Configuration) {

    /**
     * A context for [LunimaryHttpRequestRetry.Configuration.shouldRetry]
     * and [LunimaryHttpRequestRetry.Configuration.shouldRetryOnException]
     */
    class ShouldRetryContext(
        /**
    ...
    /**
     * A context for [LunimaryHttpRequestRetry.Configuration.delayMillis].
     * Contains a non-null [response] or [cause] but not both.
     */
    class DelayContext internal constructor(
    ...
    /**
     * A context for [LunimaryHttpRequestRetry.Configuration.modifyRequest].
     * Contains a non-null [response] or [cause] but not both.
     */
    class ModifyRequestContext internal constructor(
        /**
         * An original request
         */
        val request: HttpRequestBuilder,
     ...
     */
    class RetryEventData internal constructor(
        val request: HttpRequestBuilder,
        val retryCount: Int,
        val response: HttpResponse?,
        val cause: Throwable?
    ...
    /**
     * Contains [LunimaryHttpRequestRetry] configurations settings.
     */
    @KtorDsl
    class Configuration {
         ...
         */
        fun retryIf(maxRetries: Int = -1, block: ShouldRetryContext.(HttpRequest, HttpResponse) -> Boolean) {
         ...
         * Note, that in this case, [HttpTimeout] plugin should be installed after [LunimaryHttpRequestRetry].
         */
            ...
            while (true) {
                ...
                val retryData = try {
                    ...
                    if (!shouldRetryOnException(retryCount, maxRetries, shouldRetryOnException, subRequest, cause)) {
                        "shouldRetryOnException = false, ${cause.message}".logd("http_request_retry")
                        cause.printStackTrace()
                    }
                    RetryEventData(subRequest, ++retryCount,  null, null)
                }
    ...
    companion object Plugin : HttpClientPlugin<Configuration, LunimaryHttpRequestRetry> {
        override val key: AttributeKey<LunimaryHttpRequestRetry> = AttributeKey("RetryFeature")

        /**
         * Occurs on request retry.
         */
        val HttpRequestRetryEvent: EventDefinition<RetryEventData> = EventDefinition()

        override fun prepare(block: Configuration.() -> Unit): LunimaryHttpRequestRetry {
            val configuration = Configuration().apply(block)
            return LunimaryHttpRequestRetry(configuration)
        }

        override fun install(plugin: LunimaryHttpRequestRetry, scope: HttpClient) {
...
private val ShouldRetryPerRequestAttributeKey =
    AttributeKey<LunimaryHttpRequestRetry.ShouldRetryContext.(HttpRequest, HttpResponse) -> Boolean>(
        "ShouldRetryPerRequestAttributeKey"
    )

private val ShouldRetryOnExceptionPerRequestAttributeKey =
    AttributeKey<LunimaryHttpRequestRetry.ShouldRetryContext.(HttpRequestBuilder, Throwable) -> Boolean>(
        "ShouldRetryOnExceptionPerRequestAttributeKey"
    )

private val ModifyRequestPerRequestAttributeKey =
    AttributeKey<LunimaryHttpRequestRetry.ModifyRequestContext.(HttpRequestBuilder) -> Unit>(
        "ModifyRequestPerRequestAttributeKey"
    )

private val RetryDelayPerRequestAttributeKey =
    AttributeKey<LunimaryHttpRequestRetry.DelayContext.(Int) -> Long>(
